...
#dev_new["combined"] = dev_new["german_grade"] + dev_new["english_grade"]+dev_new["german_grade"]+dev_new["lang_iq"]+dev_new["logic_iq"]
train = pd.DataFrame(
    {'text': train_new["text"],
...
tfidf = TfidfVectorizer(ngram_range=(1,3), max_df=0.8966, min_df=2, sublinear_tf=False)#ngram_range=(1,config.max), min_df=config.min_df, sublinear_tf=config.sublinear_tf, max_df=config.max_df)
...
svr = SVR(C=2.425, epsilon=0.1712, gamma=0.001638)
#svr.fit(train_tfidf,train["target"])
#pred = xgbr.predict(dev_tfidf)
reg = LinearRegression()
#reg.fit(train_tfidf,train["target"])
#pred = reg.predict(dev_tfidf)
regr = RandomForestRegressor()
regr.fit(train_tfidf,train["target"])
pred = regr.predict(dev_tfidf)
...
final_df.to_csv("reg_preds_final_rf_comb.tsv",index=False, header=False,columns = ["student_ID","ranks"],sep='\t')
